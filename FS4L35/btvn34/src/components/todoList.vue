<template>
  <div id="todoListApp">
    <div id="header">
      <ul>
        <li>
          <span @click="showWeekdays = !showWeekdays">
          ...
          >
            ...
            <div @click="toDay = dayIndex;
                  showWeekdays = !showWeekdays" 
                  class="left">
                  {{ day.title }}
            </div>
            <button class="right" @click="dayList.splice(dayIndex, 1)">X</button>
          </li>
          </ul>
        </li>
        <li>
          <span
            @click="newList('New list', dayList);
    ...
    <div id="todos" v-for="(day, dayIndex) in dayList" :key="dayIndex" v-show="toDay == dayIndex">
      <div id="tienDo">
        <div>
      ...
      <ul id="todo" v-for="(todo, doIndex) in day.todos" :key="doIndex">
        <li>
          <div :class="[{green: todo.done}, {through: todo.done}]">
            <span v-show="!todo.sua">{{doIndex + 1}}. {{ todo.text }}</span>
            <input type="text" v-model="todo.text" v-show="todo.sua">
          </div>
          <div class="btns">
            <button @click="todo.done = !todo.done">
              <div>{{'Done'.repeat(!todo.done)}}{{'Undo'.repeat(todo.done)}}</div>
            </button>
            <button @click="todo.sua = !todo.sua">
              <i class="fa fa-pencil" aria-hidden="true" v-show="!todo.sua"></i>
              <span v-show="todo.sua">OK!</span>
            </button>
            <button @click="day.todos.splice(doIndex, 1)">X</button>
          </div>
        </li>
      </ul>
      <div id="doAdd" @click="day.todos.push({ text: '', done: false, sua: false})">+</div>
      ...
      dayList: [
        {
          ...
          todos: [
            {
              text:
                "Ngủ dậy đánh răng rửa mặt và ăn sáng sau đó đi ngủ them 15-20 phút nữa rồi soanj sánh vở đi học",
              done: false,
              sua: false
            },
